        ...
        if (rac) {
            ...
            $.ajax({
                ...
                success: function(jsonData) {
                    console.log('브랜드시설 : ', jsonData);
                    
                    // 데이터를 HTML 테이블에 나타내기
                    var tableHtml = "<table class='table'>";
                    tableHtml += "<tr><th>브랜드명</th><th>브랜드코드(아이콘)</th><th>세부정보</th><th>등록일</th></tr>";

                    for (var i = 0; i < jsonData.list.length; i++) {
                        var brand = jsonData.list[i];
                        tableHtml += "<tr>";
                        tableHtml += "<td>" + brand.brdName + "</td>";
                        tableHtml += "<td>" + brand.brdCode + "</td>";
                        tableHtml += "<td>" + brand.brdDesc + "</td>";
                        tableHtml += "<td>" + brand.redDtime + "</td>";
                        tableHtml += "</tr>";
                    }
                    ...
                    for (var i = 0; i < jsonData.list.length; i++) {
                        ...
                        var brandData = {
                        		...
                        		brdCode: brand.brdCode,
                        		brdDesc: brand.brdDesc,
                        		redDtime: brand.redDtime,
                        		stdRestCd: brand.stdRestCd
                        };
                        dataToSend.push(brandData);
                    }
